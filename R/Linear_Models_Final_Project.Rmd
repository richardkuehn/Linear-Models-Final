...
```{r}
print(paste("R squared value:", round(R2(predicted_classes, test_y), 4)))
```
...
```
^ Should we make a significance value to contextualize these Type 1 and Type 2 errors?
...
```
^ Might want to talk about rounding to 0 or 1 how that affects the accuracy of the model.
^ Would be interesting to visualize the accuracy when prediction < than 0.25 and > than 0.75.
^ Could count prediction between 0.25 and 0.75 as "unsure" or something along those lines.
